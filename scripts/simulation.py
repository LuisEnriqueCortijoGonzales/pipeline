...
def execute_instruction(inst):
    ...
    if op == "MOV":
        ...
        registers[inst[1]] = int(registers[inst[2]]) + (
            inst[3] if isinstance(inst[3], int) else int(registers[inst[3]])
        )
    elif op == "ADC":
        registers[inst[1]] = (
            int(registers[inst[2]])
            + (inst[3] if isinstance(inst[3], int) else int(registers[inst[3]]))
            + 1
        )
    elif op == "SUB":
        ...
        registers[inst[1]] = int(registers[inst[2]]) * int(registers[inst[3]]) + int(
            registers[inst[4]]
        )
    elif op == "MLS":
        registers[inst[1]] = int(registers[inst[2]]) * int(registers[inst[3]]) - int(
        ...
        registers[inst[1]] = int(
            (registers[inst[2]] * registers[inst[3]]) + registers[inst[4]]
        )
    elif op == "SMULL":
        registers[inst[1]] = int(registers[inst[2]] * registers[inst[3]])
    elif op == "SMLAL":
        ...
        registers[inst[1]] = (
            int(registers[inst[2]]) >> inst[3]
        )  # Sign bit not considered for simplicity
    elif op == "LSL":
        registers[inst[1]] = int(registers[inst[2]]) << inst[3]
    elif op == "ROR":
        shift = inst[3] % 32
        registers[inst[1]] = (
            (int(registers[inst[2]]) >> shift)
            | (int(registers[inst[2]]) << (32 - shift))
        ) & 0xFFFFFFFF
    elif op == "RRX":
