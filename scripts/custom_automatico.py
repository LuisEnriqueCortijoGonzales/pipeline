def arm_to_bin_hex(instruction):
    ...
    if bit_25 == "1":
        # Verificar si el valor inmediato es hexadecimal
        if operand2.startswith('#0x'):
            imm_value = int(operand2[3:], 16)  # Convertir de hexadecimal a decimal
        else:
            imm_value = int(operand2.strip('#'))  # Convertir de decimal a decimal
